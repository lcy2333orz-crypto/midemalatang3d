class_name CondimentStation
extends Area2D

@export var condiment_id: StringName = &""

@onready var _highlight: CanvasItem = $Highlight


func can_interact(player_carry: PlayerCarry) -> bool:
	if condiment_id.is_empty() or player_carry == null or not player_carry.has_item():
		return false
	...
	return bowl.can_add_condiment(condiment_id)
...
func interact(player_carry: PlayerCarry) -> bool:
	...
	if bowl == null:
		return false
	return bowl.add_condiment(condiment_id)
